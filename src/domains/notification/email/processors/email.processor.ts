...
export class EmailProcessor {
    ...
    public async handleReminderScheduleEmail(job: Job<ReminderScheduleEmailJobData>) {
        ...
        if (row.status !== ReminderScheduleStatus.QUEUED && row.status !== ReminderScheduleStatus.PENDING) {
            if (row.status === ReminderScheduleStatus.SENT || row.status === ReminderScheduleStatus.CANCELLED || row.status === ReminderScheduleStatus.EXPIRED) {
                return;
            }
            this.logger.warn(`Reminder schedule ${scheduleId} unexpected status ${row.status}, skipping`);
            return;
        }

        if (row.status === ReminderScheduleStatus.PENDING) {
            const moved = await this.reminderScheduleRepository.transitionStatus(row.id, ReminderScheduleStatus.PENDING, ReminderScheduleStatus.QUEUED);
            if (!moved) {
                return;
            }
        }

        const casOk = await this.reminderScheduleRepository.transitionStatus(row.id, ReminderScheduleStatus.QUEUED, ReminderScheduleStatus.SENDING);
        if (!casOk) {
            return;
        }
        ...
        if (fresh.reminderType === ReminderScheduleKind.ESCALATION && fresh.actedCheckAfter) {
            const acted = await this.actedCheckRegistry.hasActedSince(fresh.entityType, fresh.entityId, fresh.actedCheckAfter);
            if (acted) {
                if (fresh.initialReminderId) {
                    await this.reminderScheduleRepository.cancelSiblingEscalations(fresh.initialReminderId, fresh.id);
                }
                await this.reminderScheduleRepository.transitionStatus(fresh.id, ReminderScheduleStatus.SENDING, ReminderScheduleStatus.CANCELLED, {
        ...
        try {
            await this.emailService.sendReminderEmail(fresh.recipient, reminderTitle, fresh.template, data);
        } catch (error: unknown) {
        ...
        await this.reminderScheduleRepository.inTransaction(async (tx) => {
            const updatedCount = await this.reminderScheduleRepository.markSentIfStillSending(tx, fresh.id, sentAt);
            if (updatedCount !== 1) {
                return;
            }
            if (fresh.reminderType === ReminderScheduleKind.INITIAL) {
                const initial = await this.reminderScheduleRepository.findByIdWithClient(tx, fresh.id);
                if (initial) {
                    await this.reminderScheduleRepository.createEscalationsForInitial(tx, initial, sentAt);
                }
            }
        });
    }

    private async handleSendFailure(scheduleId: string, attempt: number, error: unknown): Promise<void> {
        const msg = error instanceof Error ? error.message : String(error);
        const code = error instanceof Error ? (error as Error & { code?: string }).code : undefined;
        const nextAttempt = attempt + 1;
        const terminal = nextAttempt >= REMINDER_CONFIG.retry.maxAttempts || code === 'EENVELOPE' || /invalid|recipient/i.test(msg);

        if (terminal) {
            await this.reminderScheduleRepository.transitionStatus(scheduleId, ReminderScheduleStatus.SENDING, ReminderScheduleStatus.FAILED_TERMINAL, {
        ...
        await this.reminderScheduleRepository.resetSendingToPendingRetry(scheduleId, nextAttempt, nextAt, code ?? 'retryable', msg);
    }
}
